...
template<class Compare>
void kravchenko::cmd::freqPredicate(CmdStreams s, const std::map<std::string, FrequencyDictionary>& data, Compare comp)
{
    std::string dictName;
    s.in >> dictName;
    ...
    {
        const FrequencyDictionary& dict = data.at(dictName);
    ...
    std::copy_n(sortedWords.cbegin(), n, OutputItT{ s.out, "\n" });
}
